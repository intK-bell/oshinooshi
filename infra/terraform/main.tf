...
# --------------------
# Ops alerts via Discord
# --------------------
resource "aws_sns_topic" "ops_alerts" {
  name = "${local.app_name}-ops-alerts-${local.env}"

  tags = local.tags
}

resource "aws_iam_role" "sns_to_discord" {
  name = "${local.app_name}-sns-to-discord-${local.env}"
...
resource "aws_iam_role_policy_attachment" "sns_to_discord_basic" {
  role       = aws_iam_role.sns_to_discord.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy" "sns_to_discord_secret_access" {
  name = "${local.app_name}-sns-to-discord-secret-${local.env}"
  role = aws_iam_role.sns_to_discord.id
...
resource "aws_lambda_function" "sns_to_discord" {
  function_name = "${local.app_name}-sns-to-discord-${local.env}"
  role          = aws_iam_role.sns_to_discord.arn
  handler       = "index.handler"
  runtime       = "python3.12"
  timeout       = 10

  s3_bucket = var.lambda_package_bucket
  s3_key    = var.sns_to_discord_package_key
  ...
  depends_on = [
    aws_iam_role_policy_attachment.sns_to_discord_basic
  ]

  tags = local.tags
}

resource "aws_sns_topic_subscription" "ops_alerts_discord" {
  topic_arn = aws_sns_topic.ops_alerts.arn
  protocol  = "lambda"
  endpoint  = aws_lambda_function.sns_to_discord.arn
}

resource "aws_lambda_permission" "allow_sns_to_discord" {
  statement_id  = "AllowExecutionFromSNS"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.sns_to_discord.function_name
  principal     = "sns.amazonaws.com"
  source_arn    = aws_sns_topic.ops_alerts.arn
}

resource "aws_cloudwatch_metric_alarm" "photo_processor_errors" {
  alarm_name          = "${local.app_name}-photo-processor-errors-${local.env}"
  ...
  alarm_description = "Lambda errors > 0 for two consecutive minutes"
...
resource "aws_cloudwatch_metric_alarm" "photo_intake_dlq_depth" {
  alarm_name          = "${local.app_name}-photo-intake-dlq-depth-${local.env}"
  comparison_operator = "GreaterThanThreshold"
  ...
  dimensions = {
    QueueName = aws_sqs_queue.photo_dlq.name
  }

  alarm_description = "DLQ has pending messages"
  alarm_actions     = [aws_sns_topic.ops_alerts.arn]
  ok_actions        = [aws_sns_topic.ops_alerts.arn]
